...
if menu == "Mutual Fund Calculator":
    ...
    if sub_menu == "SIP (Systematic Investment Plan)":
        ...
        if st.button("Calculate SIP"):
            ...
            # Display the results to the user with comma formatting
            st.write(f"Total Invested Amount: {invested_amount:,.2f}")
            st.write(f"Profit on Investment: {profit:,.2f}")
    ...
    if st.button("Calculate Loan EMI"):
        ...
        try:
            ...
            if loan_emi == 0:
                ...
                # Display the results to the user with comma formatting
                st.write(f"Loan EMI: {loan_emi:,.2f}")
    ...
    if st.button("Calculate FD Returns"):
        ...
        # Display the results to the user with comma formatting
        st.write(f"FD Total Invested Amount: {fd_invested_amount:,.2f}")
        st.write(f"FD Interest Earned: {fd_interest_earned:,.2f}")
        st.write(f"FD Total Value: {fd_total_value:,.2f}")
    ...
    if st.button("Calculate RD Returns"):
        ...
        # Display the results to the user with comma formatting
        st.write(f"RD Total Invested Amount: {rd_invested_amount:,.2f}")
        st.write(f"RD Interest Earned: {rd_interest_earned:,.2f}")
        st.write(f"RD Total Value: {rd_total_value:,.2f}")
